...
def load_augment_batch_dataset(batch_size, im_size=224):
    ...
    train_ds = train_path_labels.map(load_image_and_labels, num_parallel_calls=tf.data.AUTOTUNE).shuffle(train_len, reshuffle_each_iteration=True).map(image_augmentations, num_parallel_calls=tf.data.AUTOTUNE).batch(batch_size, drop_remainder=True).prefetch(tf.data.AUTOTUNE).map(image_cutout, num_parallel_calls=tf.data.AUTOTUNE).repeat()
    test_ds = val_path_labels.map(load_image_and_labels, num_parallel_calls=tf.data.AUTOTUNE).batch(batch_size, drop_remainder=True).prefetch(tf.data.AUTOTUNE).cache()
    return train_ds, test_ds, train_len//batch_size, val_len//batch_size
