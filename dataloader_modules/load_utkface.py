...
def load_augment_batch_dataset(batch_size, im_size=224, split_ratio=0.7):
    ...
    UTK_list_ds = UTK_list_ds.with_options(options).shuffle(UTKFace_len)
    train_size = int(split_ratio * UTKFace_len)
    train_ds = UTK_list_ds.take(train_size).shuffle(10).cache()
    test_ds = UTK_list_ds.skip(train_size).shuffle(10).cache()

    train_steps_per_epoch = train_size // batch_size
    test_steps_per_epoch = (UTKFace_len-train_steps_per_epoch) // batch_size

    train_ds = train_ds.interleave(
        lambda _: train_ds.map(load_utkface, num_parallel_calls=tf.data.AUTOTUNE).map(image_augmentations, num_parallel_calls=tf.data.AUTOTUNE).batch(batch_size, drop_remainder=True).prefetch(tf.data.AUTOTUNE),
