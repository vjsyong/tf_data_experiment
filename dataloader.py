...
#   label = tf.strings.split(tf.strings.split(file_path, "_")[0], "/")[-1]
...
def steps_per_epoch(batch_size=32):
...
def construct_mosaic(batch, batch_num):
  ...
  for ax, (image, label) in zip(grid, batch):
    ax.axis('off')
    ax.imshow(image)
  plt.savefig(f'16x16_utk_fam_{batch_num}.png', bbox_inches='tight', dpi=600)
  # plt.show()
