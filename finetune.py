...
def main():
    ...
    callbacks = [
                ...
                ModelCheckpoint(str(output_dir) + f"/weights/{model_name}-dense256/unlocked_20 layers-{batch_size}-{lr}-{opt_name}/" + f"{ds_name}" + "{epoch:03d}-{val_loss:.3f}-{val_age_mae:.3f}.hdf5",
                                 monitor="val_age_mae",
                                 verbose=1,
                                 save_best_only=True,
                                 mode="min")
